...
class GraphDataTransformer:
    ...
    def transform(self, batch_data):
        ...
        gate_indices = gate_indices.to(self.device)
        
        # Return components directly as a tuple
        return node_features, edge_index, edge_features, gate_indices
...
class TimeModel(nn.Module):
    ...
    def forward(self, x, pred_velocity):
        ...
        for layer in self.hidden_layers:
            residual = out
            out = layer(out)
            out = out + residual  # Residual connectdn
