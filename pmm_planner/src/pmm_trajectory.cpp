...
PMMTrajectory::PMMTrajectory(const Scalar ps, const Scalar vs, const Scalar pe,
  ...
  for (Vector<2> a : test_acc_vec) {
    ...
    if (v_max < MAX_SCALAR) {
      ...
      if (std::isfinite(t1_3) and std::isfinite(t2_3) and std::isfinite(t3_3) and
        ...
        if (calc_gradient) {
          ...
          const Scalar d_t_da1 = -(v_max - vs) * (v_max - vs) / (2 * pow_a1_2 * v_max);
          const Scalar d_t_da2 = (v_max - ve) * (v_max - ve) / (2 * pow_a2_2 * v_max);
        ...
        if (calc_gradient) {
          // dt/da == gradient we can use to optimize time
          const Scalar d_t_da1 = -(v_min - vs) * (v_min - vs) / (2 * pow_a1_2 * v_min);
          const Scalar d_t_da2 = (v_min - ve) * (v_min - ve) / (2 * pow_a2_2 * v_min);
    ...
    if (!found) {
      ...
      if ((vs == ve) and (((a1 - a2)*(a1*ve*ve - a2*ve*ve - 2*a1*a2*pe + 2*a1*a2*ps)) < PRECISION_DTDV_DEF)){
        // std::cout << "ERROR: PMM equations solution does not exist" << std::endl;
        continue;
  ...
  if (t1 != MAX_SCALAR and t2 != MAX_SCALAR and t3 != MAX_SCALAR and (t1 >= 0 or t2 >= 0 or t3 >= 0)) {
    exists_ = true;
...
Scalar PMMTrajectory::computeSyncTrajectory(const Scalar total_time, const Scalar a_max1, const Scalar a_max2, const Scalar max_v) {
  ...
  if (fabs(p_(0)-p_(3)) < PRECISION_PMM_VALUES and fabs(v_(0)-v_(2)) < PRECISION_PMM_VALUES){
    exists_ = true;
    trajectory_type_ = ZERO_TRAJ;
  ...
  for (Vector<2> a : test_acc_vec) {
    ...
    // if (total_time * a1 - ve + vs != 0) {
    //   const double t1 = ((-2 * vs * total_time + 2 * pe - 2 * ps) * a1 - pow((ve - vs), 2)) / (2 * a1 * (total_time * a1 - ve + vs));
    //   const double t2 = (total_time * a1 - ve + vs) / a1;
    //   const double t3 = ((2 * ve * total_time - 2 * pe + 2 * ps) * a1 - pow((ve - vs), 2)) / (2 * a1 * (total_time * a1 - ve + vs));
    //   const double v_k = vs + a1 * t1;
    ...
    // const double A = (pow(a1, 2) * pow(total_time, 2) + 2 * (ve + vs) * a1 * total_time - 4 * pe * a1 + 4 * ps * a1 - pow((ve - vs), 2));
    // if (A >= 0) {
    //   const double sqrt_A = sqrt(A);
    //   const double t1 = (a1 * total_time + ve - vs - sqrt_A) / (2 * a1);
    //   const double t2 = (sqrt_A) / a1;
    //   const double t3 = (a1 * total_time - ve + vs - sqrt_A) / (2 * a1);
    //   const double v_k = (a1 * total_time + ve + vs - sqrt_A) / 2;
    //   if (t1 > -0.0001 && t2 > -0.0001 && t3 > -0.0001 && v_k - v_min > -0.0001 && v_k - v_max < 0.0001) {
    //     used_a1 = a1;
    //     used_a2 = a2;
    //     exists_ = true;
    ...
    if (fabs(vs-ve) < PRECISION_PMM_VALUES){
      ...
      t2_3 = (2 * pe + 2 * ps + total_time * (v_max + ve)) / (v_max - ve);
      t2_4 = (2 * pe + 2 * ps + total_time * (v_min + ve)) / (v_min - ve);
      t1_3 = -(a2*(total_time - t2_3))/(a1 - a2);
      t1_4 = t1_3;
      t3_3 = (a1*(total_time - t2_4))/(a1 - a2);
      t3_4 = t3_3;
      sc_3 = ((a1 - a2) * pow(v_max - ve, 2)) / (2 * a1 * a2 * (pe - ps - total_time * v_max));
      sc_4 = ((a1 - a2) * pow(v_min - ve, 2)) / (2 * a1 * a2 * (pe - ps - total_time * v_min));
