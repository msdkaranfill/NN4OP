...
namespace pmm {
    ...
    //Initializes an instance of PMM_MG_Trajectory3D with (n_path_segments) and (v_max).
    PMM_MG_Trajectory3D::PMM_MG_Trajectory3D(const int n_path_segments, const Scalar v_max) : n_segments(n_path_segments) {
    ...
    PMM_MG_Trajectory3D::PMM_MG_Trajectory3D(std::vector<Vector<3>> p, std::vector<Vector<3>> v,
                                             Vector<3> a_max, const Scalar v_max) : n_segments(p.size()-1){
        _dt.resize(3 * n_segments);
        _dtdv.resize(2 * n_segments);
        _p.resize(3 *n_segments +1);
        _v.resize(3 *n_segments +1);
        ...
        for(int i=0; i<n_segments; i++){
            ...
            PointMassTrajectory3D segment_tr(start_conf, end_conf, a_max, v_max_, true, true);    // equalize time
    ...
    //mp_tr(path_waypoints, gates_velocities, min_per_axis_acc_vec, max_per_axis_acc_vec, max_velocity_per_axis);
    PMM_MG_Trajectory3D::PMM_MG_Trajectory3D(std::vector<Vector<3>> p, std::vector<Vector<3>> v,
                                             Vector<3> a_min, Vector<3> a_max, const Scalar v_max) : n_segments(p.size()-1){
        _dt.resize(3 * n_segments);
        _dtdv.resize(2 * n_segments);
        _p.resize(3 *n_segments +1);
        _v.resize(3 *n_segments +1);
        ...
        for(int i=0; i<n_segments; i++){
            ...
            PointMassTrajectory3D segment_tr(start_conf, end_conf, a_max, a_min, v_max_, equalize_times, calculate_gradients);
    ...
    PMM_MG_Trajectory3D::PMM_MG_Trajectory3D(std::vector<Vector<3>> p, std::vector<Vector<3>> v,
                                             Vector<3> a_min, Vector<3> a_max, const Scalar v_max, bool equalize_axes) : n_segments(p.size()-1){
        _dt.resize(3 * n_segments);
        _dtdv.resize(2 * n_segments);
        _p.resize(3 *n_segments +1);
        _v.resize(3 *n_segments +1);
        ...
        for(int i=0; i<n_segments; i++){
            ...
            PointMassTrajectory3D segment_tr(start_conf, end_conf, a_max, a_min, v_max_, equalize_axes, calculate_gradients);
    ...
    PMM_MG_Trajectory3D::PMM_MG_Trajectory3D(std::vector<Vector<3>> p, std::vector<Vector<3>> v, const Scalar max_acc_norm, const Scalar v_max,
                                             const int td_max_iter, const Scalar td_acc_norm_precision_limit, const bool debug) : n_segments(p.size()-1){
        _dt.resize(3 * n_segments);
        _dtdv.resize(2 * n_segments);
        _p.resize(3 *n_segments +1);
        _v.resize(3 *n_segments +1);
        ...
        for(int i=0; i<n_segments; i++){
            ...
            PointMassTrajectory3D segment_tr(start_conf, end_conf, max_acc_norm, v_max_, td_max_iter, td_acc_norm_precision_limit, debug);
    ...
    PMM_MG_Trajectory3D::PMM_MG_Trajectory3D(std::vector<Vector<3>> waypoints, const Vector<3> start_vel, const Vector<3> end_vel,
                                            const Scalar max_acc_norm, const Scalar v_max, const Scalar dT_precision, const int GD_max_iter, 
                                            ...
                                            const Scalar GD_alpha_min_thr_2nd_rnd, const bool debug) : n_segments(waypoints.size()-1){
        ...
        std::vector<Vector<3>> init_vel = compute_initial_velocities(waypoints, start_vel, end_vel, max_per_axis_acc_vec[0], v_max_);

        // compute and optimize trajectory
        PMM_MG_Trajectory3D mp_tr(waypoints, init_vel, min_per_axis_acc_vec, max_per_axis_acc_vec, v_max_);
        if (debug){std::cout << "Original trajectory duration: " << mp_tr.duration() << " s." << std::endl;}
        mp_tr.optimize_velocities_at_positions(GD_alpha, GD_alpha_red_fctr, GD_alpha_min_thr, GD_max_iter, dT_precision);

        // recompute trajectory using limited thrust decomposition
        std::vector<Vector<3>> v = mp_tr.get_current_velocities();
        PMM_MG_Trajectory3D mp_tr_o(wpts, v, max_acc_norm, v_max_, TD_max_iter, TD_acc_precision, false);
        ...
        if (scnd_round_optim){
            mp_tr_o.optimize_velocities_at_positions(GD_alpha_2nd_rnd, GD_alpha_red_fctr_2nd_rnd, GD_alpha_min_thr_2nd_rnd,
                                                    GD_max_iter_2nd_rnd, dT_precision);
    ...
    int PMM_MG_Trajectory3D::optimize_velocities_at_positions(Scalar gd_step, Scalar gd_step_reduction_factor, Scalar gd_step_min_threshold,
                                            int max_iter, Scalar dT_threshold, int single_axis_optim_idx, bool debug,
        ...
        for (iter=0; iter < max_iter; iter++) {
            ...
            for (int seg = 1; seg < (this->n_segments); seg++){
                ...
                int succ = this->updateTrajectoryGivenVelocities(v_new[3*seg_idx], seg_idx, equalize_time, true);

                if (debug){
                    // Check feasibility
                    bool res = this->checkTrajectory();
                    if (!res){
                ...
                if ((this->segment_duration(seg_idx-1, 0) + this->segment_duration(seg_idx, 0)) > (seg1_time_old + seg2_time_old + PRECISION_PMM_VALUES) or (succ==-1)){
                    ...
                    this->updateTrajectoryGivenVelocities(v_new[3*seg_idx], seg_idx, equalize_time, true);
                    seg--;
                } else {
            ...
            if (T_new > T_last){
                this->updateTrajectoryGivenVelocities(v_last, equalize_time, true);
                if (debug){
        ...
        for (int seg_idx=0; seg_idx< n_segments; seg_idx++){
            for (int ax=0; ax<3;ax++){
                ...
                if (_thrust_limit_flag){
                    if ((_a[(3*seg_idx)]- GVEC).norm() > (_max_acc_norm + _TD_norm_precision) or 
                        (_a[(3*seg_idx+1)]- GVEC).norm() > (_max_acc_norm + _TD_norm_precision) or (_a[(3*seg_idx)+2]- GVEC).norm() > (_max_acc_norm + _TD_norm_precision)){
    ...
    int PMM_MG_Trajectory3D::fillTrajectorySegment(PointMassTrajectory3D *tr, int segment_idx, const bool override){
        if ((segment_idx < this->n_segments_filled) and (!override)){
            ...
            std::cout << "segment_idx" << segment_idx << "this->n_segments_filled" << this->n_segments_filled << std::endl;
        ...
        if (!tr->exists()){
            ...
            //std::cout << tr->debugInfo() << std::endl;
    ...
    int PMM_MG_Trajectory3D::updateTrajectoryGivenVelocities(std::vector<Vector<3>> v_new, const bool equalize_times, const bool sync_traj){
        if (n_segments_filled != n_segments){
        ...
        for (int i = 0; i < n_segments; i++) {
            ...
            if (!sync_traj){
                PointMassTrajectory3D tr(start_conf, end_conf, _a_max, v_max_, equalize_times, true);    // equalize time
                if (!tr.exists()){
                    return -1;
                }
                ...
                if (_thrust_limit_flag){
                    PointMassTrajectory3D tr(start_conf, end_conf, _max_acc_norm, v_max_, _TD_max_iter, _TD_norm_precision, false);
                    if (!tr.exists()){
                        return -1;
                    }
                    succ *= fillTrajectorySegment(&tr, i);
                } else {
                    PointMassTrajectory3D tr(start_conf, end_conf, _a_max, v_max_, equalize_times, true);
                    if (!tr.exists()){
                        return -1;
                    }
                    succ *= fillTrajectorySegment(&tr, i);
    ...
    int PMM_MG_Trajectory3D::updateTrajectoryGivenVelocities(Vector<3> v_new, const int segment_idx, const bool equalize_times, const bool sync_traj){
        if (n_segments_filled != n_segments){
            // The trajectory is not complete, some data is missing
            std::cout << "ERROR:updateTrajectoryGivenVelocities method failed due to incomplete trajectory." << std::endl;
            return 0;
        ...
        for (int i = 0; i < 2; i++) {
            ...
            if (!sync_traj){
                ...
                if (_thrust_limit_flag){
                    ...
                    PointMassTrajectory3D tr(start_conf, end_conf, _a_max, _a_min, v_max_, equalize_times, true);
                    if (!tr.exists()){
                        return -1;
                    }
                    succ *= fillTrajectorySegment(&tr, (segment_idx-1)+i, true);
    ...
    std::tuple<Scalar, Scalar, Scalar>  PMM_MG_Trajectory3D::get_MIN_SYNC_grad_and_bounds(const int ax_idx, const int min_idx){
        ...
        Scalar b_min = -v_max_; Scalar b_max = v_max_;
    ...
    std::tuple<Scalar, Scalar, Scalar> PMM_MG_Trajectory3D::get_SYNC_MIN_grad_and_bounds(const int ax_idx, const int min_idx){
     ...
     std::tuple<Scalar, Scalar, Scalar> PMM_MG_Trajectory3D::get_MIN_MIN_grad_and_bounds(const int ax_idx, const int second_min_idx){
        ...
        b_min = std::max(b_min, -v_max_);
        b_max = std::min(b_max, v_max_);
    ...
    std::tuple<Scalar, Scalar> PMM_MG_Trajectory3D::get_SYNC_MIN_vel_bounds(const int seg_idx, const int ax_idx){
        ...
        Scalar v12_min = -v_max_;
        Scalar v12_max = v_max_;
    ...
    std::tuple<Scalar, Scalar> PMM_MG_Trajectory3D::get_MIN_SYNC_vel_bounds(const int seg_idx, const int ax_idx){
        ...
        Scalar v0_min = -v_max_;
        Scalar v0_max = v_max_;
    ...
    PMM_MG_Trajectory3D PMM_MG_Trajectory3D::operator=(PMM_MG_Trajectory3D tr){
        return {tr};
    }
    ...
    std::vector<Vector<3>> compute_initial_velocities(std::vector<Vector<3>> waypoints, Vector<3> start_velocity,
                     Vector <3> end_velocity, const Scalar max_per_axis_acc, const Scalar v_max, const Scalar const_vel_norm){
        ...
        for (int i = 1; i<n_waypoints-1; i++){
            ...
            if (v_new.cwiseAbs().maxCoeff() > v_max){
                v_new *= v_max / v_new.cwiseAbs().maxCoeff();
            }
